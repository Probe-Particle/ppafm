...
class RelaxedScanner:
    ...
    def run_getFEinStrokes(self, FEout=None, FEconv=None, FEin=None, lvec=None, nz=None, WZconv=None, bDoConv=False):
        ...
        kargs = (self.cl_ImgIn, self.cl_poss, self.cl_FEout, self.invCell[0], self.invCell[1], self.invCell[2], self.dTip, self.dpos0, np.int32(nz))
    ...
    def run_convolveZ(self, FEconv=None, nz=None):
        ...
        if FEconv is None:
            if self.FEconv is not None:
                ...
                FEconv = self.prepareFEConv()
        kargs = (self.cl_FEout, self.cl_FEconv, self.cl_WZconv, np.int32(nz), np.int32(self.nDimConvOut))
