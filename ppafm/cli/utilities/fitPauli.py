...
for ie in range(number_of_original_elements):
    ...
    if options.old: # old verison of atomtypes.ini
        f2.write(str(tmp[0])+' '+str(tmp[1])+' '+str(ie+1+number_of_atoms)+' '+str(tmp[3].decode('UTF-8'))+'\n')
    else: # ocl version of atomtypes.ini
        f2.write(str(tmp[0])+' '+str(tmp[1])+' '+str(tmp[2])+' '+str(ie+1+number_of_atoms)+' '+str(tmp[4]B.decode('UTF-8'))+'\n')
