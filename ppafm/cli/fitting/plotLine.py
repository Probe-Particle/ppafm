        ...
        if (current_pos >= MIN).all() and (current_pos <= MAX).all():
            result.append(
                np.array(
                    [
                        norm_direction * i,
                        interp([current_pos[2], current_pos[1], current_pos[0]])[0],
                        current_pos[0],
                        current_pos[1],
                        current_pos[2],
                    ]
...
for p in options.points:
    ...
    if opt_dict["disp"]:
        ...
        DSPplot = selectLine(
            ...
            MIN=scan_min,
            MAX=scan_max,
            startingPoint=np.array([xmin, ymin, zmin]),
        ...
        plt.ylabel(
            r"PP $\Delta$ {} displacement ($\AA$)".format(opt_dict["disp"][0]),
            color="black",
        )
        plt.show()
    ...
    if not opt_dict["nodisp"]:
        ...
        while i < len(POSplot[0]):
            perplane.scatter(
                x=int((POSplot[0][i] - scan_min[0]) / scan_step[0]),
                y=int((POSplot[1][i] - scan_min[1]) / scan_step[1]),
                s=50,
                c="red",
                alpha=0.8,
            )
