...
if __name__=="__main__":
    ...
    try: #wrote in this way in can work with both LJ and Pauli+vdW modes
        FFpauli, lvec, nDim, atomic_info_or_head = io.load_vec_field( "FFpauli" ,data_format=args.data_format)
        FFpauli[0,:,:,:],FFpauli[1,:,:,:] = rotFF( FFpauli[0,:,:,:],FFpauli[1,:,:,:], opt_dict['rotate'] )

        print(" load Lenard-Jones Force-field ")
        FFvdW, lvec, nDim, atomic_info_or_head = io.load_vec_field( "FFvdW" , data_format=args.data_format)
        FFvdW[0,:,:,:],FFvdW[1,:,:,:] = rotFF( FFvdW[0,:,:,:],FFvdW[1,:,:,:], opt_dict['rotate'] )
    except OSError:
        print(" load Lenard-Jones Force-field ")
        FFvdW, lvec, nDim, atomic_info_or_head = io.load_vec_field( "FFLJ" , data_format=args.data_format)
        FFvdW[0,:,:,:],FFvdW[1,:,:,:] = rotFF( FFvdW[0,:,:,:],FFvdW[1,:,:,:], opt_dict['rotate'] )
