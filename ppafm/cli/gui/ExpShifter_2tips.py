...
class ApplicationWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        # -------------- Potential
        vb = QtWidgets.QHBoxLayout(); l0.addLayout(vb); vb.addWidget( QtWidgets.QLabel("CO") )
        ...
        bx = QtWidgets.QDoubleSpinBox();  bx.setSingleStep(0.1); bx.setValue(0.0); bx.setRange(0.0,1.0); bx.valueChanged.connect(self.selectAlpha); vb.addWidget(bx); bx.setToolTip('select level of overlay for smaller size tips AFM image'); bx.setEnabled(False); self.bxAlpha=bx
        ...
        vb = QtWidgets.QHBoxLayout();        l0.addLayout(vb);
        bt = QtWidgets.QPushButton('Load .npz', self); bt.setToolTip('load .npz file  from dir'); bt.clicked.connect(self.loadNPZ ); vb.addWidget( bt ); self.btLoad2 = bt; self.btLoad2.setEnabled(False)
    ...
    def loadNPZ(self):
        ...
        self.marginW.blockSignals(True); self.marginW.setValue( self.margins[2]); self.marginW.blockSignals(False);
        self.marginY0.blockSignals(True); self.marginY0.setValue( self.margins[1]); self.marginY0.blockSignals(False);
        self.marginH.blockSignals(True); self.marginH.setValue( self.margins[3]); self.marginH.blockSignals(False);
        self.updateDataView()
        self.bxChannel.setEnabled(True)
        self.bxZ.setEnabled(True)
        self.bxX.setEnabled(True)
    ...
    def shiftData(self):
        ...
        iy = int(self.bxY.value()); diy = iy - self.shifts[self.tip][iz][1]; self.shifts[self.tip][iz][1] = iy
    ...
    def updateDataView(self):
        ...
        if (self.tip == self.ind_biggest_AFM):
            ...
            big_len_image[self.margins[self.tip][1] :self.margins[self.tip][1] +self.margins[self.tip][3],self.margins[self.tip][0] :self.margins[self.tip][0]+self.margins[self.tip][2]] = small_len_image
