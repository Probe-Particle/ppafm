...
#rho1, lvec1, nDim1, head1 = loadXSF("./pyridine/CHGCAR.xsf")
#rho2, lvec2, nDim2, head2 = loadXSF("./CO_/CHGCAR.xsf")

print(">>> Loading sample from ", options.sample, " ... ")
rhoS, lvecS, nDimS, headS = loadXSF( options.sample )
print(">>> Loading tip from ", options.tip, " ... ")
rhoT, lvecT, nDimT, headT = loadXSF( options.tip    )
...
if options.Bpower > 0.0:
    ...
    if options.saveDebugXsfs:
        saveXSF( "sample_density_pow_%03.3f.xsf" %B, rhoS, lvecS, head=headS )
        saveXSF( "tip_density_pow_%03.3f.xsf" %B, rhoT, lvecT, head=headT )
...
if options.energy:
    saveXSF( "E"+namestr+".xsf", E*(PQ*-1.0), lvecS, head=headS )
saveXSF( "FF"+namestr+"_x.xsf", Fx*PQ,       lvecS, head=headS )
saveXSF( "FF"+namestr+"_y.xsf", Fy*PQ,       lvecS, head=headS )
saveXSF( "FF"+namestr+"_z.xsf", Fz*PQ,       lvecS, head=headS )
