...
def main(argv=None):
    ...
    parser.add_argument(
        ...
        type=float,
        default=0,
        help="Tilt of tip electrostatic field (radians)",
    )
    parser.add_argument(
        "--KPFM_tip",
        action="store",
    ...
    parser.add_argument(
        ...
        action="store",
        type=float,
        help="Field under the KPFM dens. and Vh was calculated in V/Ang",
    )
    parser.add_argument(
        "--z0",
        action="store",
    ...
    if args.KPFM_sample is not None:
        ...
        if geometry_format == "xsf" and args.KPFM_sample.lower().endswith(".xsf"):
            Vref_s = args.Vref
            print(
                ">>> Loading Hartree potential under bias from ",
                args.KPFM_sample,
                "...",
            )
            print("Use loadXSF")
        ...
        if geometry_format == "xsf" and args.KPFM_tip.lower().endswith(".xsf"):
            ...
            "dipole",
            "pz",
        }:  # To be put on a library in the near future...
            Vref_t = -0.1
        ...
        FFkpfm_t0sV, Eel_t0sV = PPH.computeElFF(
            dV_kpfm,
            lvec,
            nDim,
            tip_aux_2,
        ...
        FFkpfm_tVs0, Eel_tVs0 = PPH.computeElFF(
            V_v0_aux2,
            lvec,
            nDim,
            drho_kpfm,
