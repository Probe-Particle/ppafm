...
class Corrector():
    ...
    def debug_plot(self, itr, AFM_Err, AFMs, AFMRef, Err ):
        if self.logImgName is not None:
            ...
            plt.subplot(1,3,1); plt.imshow( AFMRef[:,:,self.izPlot], origin='image' ); #plt.title("AFMref" ); plt.grid()
            plt.subplot(1,3,2); plt.imshow( AFMs  [:,:,self.izPlot], origin='image', vmin=vmin, vmax=vmax ); #plt.title("AFM[]"  ); plt.grid()
            plt.subplot(1,3,3); plt.imshow( np.sqrt(AFM_Err[:,:,self.izPlot]), origin='image', vmin=0, vmax=np.sqrt(v2max)*0.2 ); #plt.title("AFMdiff"); plt.grid()
