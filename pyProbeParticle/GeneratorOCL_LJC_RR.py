...
class Generator(Sequence,):
    ...
    def prepareRFF(self,natom):
        ...
        self.rff_natom = natom
        rff.ralloc(natom)
        self.rff_types  = rff.getTypes(natom)
        self.rff_poss   = rff.getPoss(natom)
        self.rff_qrots  = rff.getQrots(natom)
        self.rff_hbonds = rff.getHbonds(natom)
        self.rff_ebonds = rff.getEbonds(natom)
        self.rff_caps   = rff.getBondCaps(natom)
    ...
    def genMolRffRandom(self):
        ...
        self.rff_qrots[:,:]  = np.random.rand(self.rff_natom,4)-0.5
        rs          = np.sum(self.rff_qrots**2, axis=1 )
        self.rff_qrots      /= rs[:,None]

        rff.setBox( p0=np.array([-5.0,-5.0,-1.0]), p1=np.array([5.0,5.0,1.0]), K=-1.0, fmax=1.0  )
        rff.setSurf(K=-0.2, x0=0.0, h=np.array([0.0,0.0,1.0]) )

    def relaxMolRff(self):
        t1 = time.clock();
