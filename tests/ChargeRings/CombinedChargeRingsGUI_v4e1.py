...
class ApplicationWindow(GUITemplate):
    ...
    def plot_voltage_line_scan_exp(self, start, end, pointPerAngstrom=5):
        ...
        # Plot probabilities if requested
        if self.cbShowProbs.isChecked():
            figp = plt.figure(); nS = probs_arr.shape[2]; axs = figp.subplots( (nS+1)//2, 2 ).flatten()
            for i in range(nS): pu.plot_imshow(axs[i], probs_arr[:,:,i], title=f"P{i}", extent=[0,dist,0,Vbiases[-1]], cmap='viridis')
            figp.show()
    ...
    def run_voltage_scan_p1p2(self):
        ...
        if self.cbShowProbs.isChecked():
            figp = plt.figure(); nS = probs_arr.shape[2]; axs = figp.subplots( (nS+1)//2, 2 ).flatten()
            for i in range(nS): pu.plot_imshow(axs[i], probs_arr[:,:,i], title=f"P{i}", extent=[0,dist,0,0.6], cmap='viridis')
            figp.show()
