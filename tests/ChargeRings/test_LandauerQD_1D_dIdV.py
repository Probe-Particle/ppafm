...
if use_occupancy:
    ...
    Q_qds,_,_ = chr.solveSiteOccupancies(ps_line, Qtips)
    ...
    # Calculate dI/dV map using pre-computed Hamiltonians
    didv_map = system.scan_didv_2D(ps_line, energies, V_bias, dV, H_QDs=H_QDs, T=T)
else:
...
# Plot dI/dV map with eigenvalue bands
ax1 = fig.add_subplot(gs[0, 0])
im = ax1.imshow(didv_map.T, aspect='auto', origin='lower', 
                extent=[0, len(ps_line), energies[0], energies[-1]], 
                cmap='cividis')
...
ax1.set_title('dI/dV Map and Energy Levels')

# Add colorbar
cbar_ax1 = fig.add_subplot(gs[0, 1])
plt.colorbar(im, cax=cbar_ax1, label='dI/dV')

# Plot dI/dV at selected energies
...
for E in selected_energies:
    ...
    ax2.plot(didv_map[:, idx], label=f'E = {E:.1f} eV')
    ax1.axhline(E, ls=':', c='k', alpha=0.5)
ax2.set_xlabel('Position Index')
ax2.set_ylabel('dI/dV')
ax2.legend()
ax2.set_title('dI/dV vs Position at Selected Energies')
plt.grid()
