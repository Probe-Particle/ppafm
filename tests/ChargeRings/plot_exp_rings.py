...
# First scan to get dimensions
fname_test = path+'TBTPA on Pb1110042.sxm'  # Using first file to get dimensions
scan_test = nap.read.Scan(fname_test)
ny, nx = scan_test.signals['X']['forward'].shape

# Initialize 3D numpy arrays
all_X = np.zeros((NPoint, ny, nx))
all_Y = np.zeros((NPoint, ny, nx))
all_dIdV = np.zeros((NPoint, ny, nx))
all_I = np.zeros((NPoint, ny, nx))
all_biases = np.zeros(NPoint)

# Arrays for voltage dependencies
dIdV_vs_V = np.zeros(NPoint)
I_vs_V = np.zeros(NPoint)
if n2ndpoint:
    dIdV_vs_V2 = np.zeros(NPoint)
    I_vs_V2 = np.zeros(NPoint)

center_x = -62.97  # Center position
center_y = 82.39
...
with open(fname_out+'.dat','w') as fout:
	...
	for i in [42,41,40,39,38,43,44,45,46,47,48,49,50,51]: ## ordered by increasing voltage
		...
		# Shift coordinates relative to center point
		X_A_shifted = X_A - (-62.97)
		Y_A_shifted = Y_A - 82.39
		...
		if PlotCurrent:
			...
			p1 = ax[a,b].imshow(dIdV_A,aspect='equal',cmap='seismic',vmin=-0.7, vmax=0.7, extent =[xmin,xmax,ymin,ymax])
		...
		# Store values for voltage dependency plots
		dIdV_vs_V[k] = dIdV_A[idxy,idxx]
		I_vs_V[k] = I_A[idxy,idxx]
		
		if n2ndpoint:
			dIdV_vs_V2[k] = dIdV_A[idxy2,idxx2]
			I_vs_V2[k] = I_A[idxy2,idxx2]
...
if PlotCurrent:
	ax[a,b+1].plot(all_biases, I_vs_V, color='#008800', linewidth=2.0,linestyle='solid', marker='o', markersize=4)
else: ## dI/dV
	ax[a,b+1].plot(all_biases, dIdV_vs_V, color='#008800', linewidth=2.0,linestyle='solid', marker='o', markersize=4)
...
if n2ndpoint:
	if PlotCurrent:
		ax[a,b+2].plot(all_biases, I_vs_V2, color='royalblue', linewidth=2.0,linestyle='solid', marker='o', markersize=4)
	else: ## dI/dV
		ax[a,b+2].plot(all_biases, dIdV_vs_V2, color='royalblue', linewidth=2.0,linestyle='solid', marker='o', markersize=4)
...
save_path = 'exp_rings_data.npz'
np.savez(save_path,
         X=all_X,        # Now these are already 3D arrays
         Y=all_Y,
         dIdV=all_dIdV,
         I=all_I,
         **metadata)
print(f"\nData saved to {save_path}")
print(f"Data shapes: X:{all_X.shape}, dIdV:{all_dIdV.shape}")
