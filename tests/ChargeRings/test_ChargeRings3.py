...
# def makePosQscan_(ps, qs):
#     npoint = ps.shape[0]    # number of positions
#     ncharge = len(qs)       # number of charges

#     # For positions: expand and broadcast
#     Ps = np.expand_dims(ps, axis=0)                  # shape: (1, npoint, 3)
#     Ps = np.broadcast_to(Ps, (ncharge, npoint, 3))   # shape: (ncharge, npoint, 3)

#     # For charges: expand and broadcast
#     Qs = np.expand_dims(qs, axis=1)                  # shape: (ncharge, 1)
#     Qs = np.broadcast_to(Qs, (ncharge, npoint))      # shape: (ncharge, npoint)
#     print("npoint ", Ps.shape, " ncharge ", ncharge )
#     print("Ps.shape ", Ps.shape, " Qs.shape", Qs.shape)
#     return Ps, Qs
...
ps_line = getLine(spos, [0.5,0.5,-3.0], [-3.0,-3.0,1.0], n=100 )
ps_line[:,2] = z_tip
...
ps, qs = makePosQscan ( ps_line, Qtips )
#ps, qs = makePosQscan_( ps_line, Qtips )
