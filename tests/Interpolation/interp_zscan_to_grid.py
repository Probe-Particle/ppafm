...
def plot_single_slice(xs, ys, zs, vol, plot_slice_z=None, show=True, save_prefix=None):
    ...
    if out_dir is not None:
        fname = os.path.join(out_dir, f"slice_z{zs[iz]:.3f}.png")
        fig.savefig(fname, dpi=150)
    if show:
        plt.show()
    else:
...
if __name__ == "__main__":
    ...
    p.add_argument("--points", required=True, help="Clean points file (type x y or index type x y)")
    p.add_argument("--zscan", required=True, help="Z-scan data file (pNNN zMM value)")
    p.add_argument("--out-npy", required=True, help="Output .npy file for 3D volume (nz, ny, nx)")
    p.add_argument("--nx", type=int, default=50, help="Number of grid points in x")
    p.add_argument("--ny", type=int, default=50, help="Number of grid points in y")
    p.add_argument("--nz", type=int, default=None, help="Number of grid points in z (default: use all from input)")
    p.add_argument("--z0", type=float, default=1.6, help="Starting z value (offset)")
    p.add_argument("--dz", type=float, default=0.1, help="Grid spacing in z")
    p.add_argument("--R-basis", type=float, default=1.2, help="Support radius for RBF/Kriging kernel")
    p.add_argument("--kind", choices=["rbf", "kriging"], default="rbf", help="Interpolator type")
    p.add_argument("--plot-slice-z", type=float, default=None, help="Optional single z-value to plot nearest slice with imshow")
    p.add_argument("--zmin", type=float, default=None, help="Optional minimum z for sequence of slices (linear interp in z)")
    p.add_argument("--zmax", type=float, default=None, help="Optional maximum z for sequence of slices (linear interp in z)")
    p.add_argument("--zstep", type=float, default=None, help="Optional z step for sequence of slices (linear interp in z)")
    p.add_argument("--show", action="store_true", help="Show matplotlib plot for selected slice(s)")
    p.add_argument("--save-prefix", type=str, default=None, help="If given, save plotted slice(s) as PNG with this prefix")
    args = p.parse_args()
